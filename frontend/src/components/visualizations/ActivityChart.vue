...
function draw() {
  ...
  const data = viewMode.value === 'weekly' ? aggregateWeekly(rawActivity) : rawActivity
  ...
  const xAxis = d3.axisBottom(x)
    .tickValues(data.filter((d, i) => i % tickInterval === 0).map(d => d.date))
    .tickFormat(d => {
  ...
  // Y axis
  g.append('g')
    .call(d3.axisLeft(y).ticks(5).tickFormat(d => Number.isInteger(d) ? d : ''))
    .call(g => g.select('.domain').attr('stroke', '#334155'))
    .call(g => g.selectAll('text').style('font-size', '11px').style('fill', '#64748b'))
    .call(g => g.selectAll('.tick line').attr('stroke', '#334155'))

  // Color scale by count (low = muted blue, high = bright blue)
  const maxCount = d3.max(data, d => d.count)
  const barColor = d3.scaleSequential(d3.interpolateBlues).domain([0, maxCount * 1.5])
    ...
    <div class="flex flex-wrap items-center gap-4 mb-6">
      ...
      <div class="flex rounded-lg overflow-hidden border border-gray-700 shrink-0">
        <button
          ...
          :class="viewMode === 'daily' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'"
          class="px-3 py-1.5 text-sm transition"
        >Daily</button>
        <button
          @click="viewMode = 'weekly'"
          :class="viewMode === 'weekly' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'"
          class="px-3 py-1.5 text-sm transition"
        >Weekly</button>
      </div>
      ...
      <div class="ml-auto flex gap-6 text-sm text-gray-400">
        ...
        <div>Active days <span class="text-white font-semibold">{{ totalDays }}</span></div>
      </div>
    </div>
