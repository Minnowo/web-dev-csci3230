...
function rebuildActiveArrays() {
  allNodes = [...rawNoteNodes, ...(showTagNodes.value ? rawTagNodes : [])];
  allLinks = [
...
const visibleNodeIds = computed(() => {
  ...
  nodes = nodes.filter(
    (n) => n.type === "tag" || n.connectionCount >= minConnections.value,
  );
...
const isolatedCount = computed(
  () =>
    visibleNoteNodes.value.filter(
      (n) => (effectiveDegree.value.get(n.id) ?? 0) === 0,
    ).length,
);
...
const mostUsedTag = computed(() => {
  let best = null,
    bestCount = 0;
  tagFrequency.value.forEach((count, tag) => {
    if (count > bestCount) {
      bestCount = count;
      best = tag;
    }
...
const mostVersatileNote = computed(() => {
  ...
  return visibleNoteNodes.value.reduce(
    (best, n) => (n.tags.length > (best?.tags.length ?? -1) ? n : best),
    null,
  );
});
...
const newTagsLast30Days = computed(() => {
  ...
  const recent = visibleNoteNodes.value.filter(
    (n) => n.created_at && new Date(n.created_at) > cutoff,
  );
  const older = visibleNoteNodes.value.filter(
    (n) => !n.created_at || new Date(n.created_at) <= cutoff,
  );
...
const statsNoteCount = computed(() => {
  ...
  return allNodes.filter((n) => n.type === "note" && visibleIds.has(n.id))
    .length;
});
const statsTagCount = computed(() => {
  const visibleIds = effectiveVisibleIds.value;
  return allNodes.filter((n) => n.type === "tag" && visibleIds.has(n.id))
    .length;
});
...
const lastUpdatedNote = computed(() => {
  ...
  const notes = allNodes.filter(
    (n) => n.type === "note" && visibleIds.has(n.id) && n.updated_at,
  );
...
const allClusters = computed(() => {
  ...
  visibleLinks.value
    ...
    .forEach((l) => {
      const s = l.source?.id ?? l.source;
      const t = l.target?.id ?? l.target;
      if (noteIds.has(s) && noteIds.has(t)) {
        adj.get(s).push(t);
        adj.get(t).push(s);
      }
    });
...
watch(focusedCluster, (val) => {
  if (!val) selectedClusterIndex.value = null;
});
watch(allClusters, () => {
...
function yearColor(dateStr) {
  ...
  const years = [
    ...new Set(noteNodes.map((n) => new Date(n.created_at).getFullYear())),
  ].sort();
...
function updateTimelineOverlay(transform, width, height) {
  ...
  const allYears = d3.timeYear.range(
    timelineScale.domain()[0],
    timelineScale.domain()[1],
  );
  const noteNodes = simNodes.filter((n) => n.type === "note" && n.created_at);
  const sortedYears = [
    ...new Set(noteNodes.map((n) => new Date(n.created_at).getFullYear())),
  ...
  timelineAxisGroup
    ...
    .attr("y", 0)
    .attr("width", (d) =>
      Math.max(0, rescaled(d3.timeYear.offset(d, 1)) - rescaled(d)),
    )
    .attr("height", axisH)
  ...
  timelineAxisGroup
    ...
    .style("fill", (d) => {
      const idx = sortedYears.indexOf(d.getFullYear());
      return YEAR_COLORS[idx % YEAR_COLORS.length];
    })
  ...
  if (k >= 0.35) {
    const months = d3.timeMonth
      .range(d3.timeMonth.floor(d0), d3.timeMonth.ceil(d1))
      .filter(
        (m) => m > timelineScale.domain()[0] && m < timelineScale.domain()[1],
      );

    gridG
      .selectAll(".tl-month-line")
      .data(months)
      .join("line")
      .attr("class", "tl-month-line")
      .attr("x1", (d) => rescaled(d))
      .attr("x2", (d) => rescaled(d))
      .attr("y1", 22)
      .attr("y2", axisH)
    ...
    if (k >= 0.7) {
      gridG
        .selectAll(".tl-month-label")
        .data(months)
        .join("text")
        .attr("class", "tl-month-label")
        .attr("x", (d) => rescaled(d) + 3)
        .attr("y", 32)
  ...
  if (k >= 1.8 && visibleDays <= 90) {
    const days = d3.timeDay
      .range(d3.timeDay.floor(d0), d3.timeDay.ceil(d1))
      .filter(
        (d) => d > timelineScale.domain()[0] && d < timelineScale.domain()[1],
      );

    gridG
      .selectAll(".tl-day-line")
      .data(days)
      .join("line")
    ...
    if (visibleDays <= 60) {
      gridG
        .selectAll(".tl-day-label")
        .data(days)
        .join("text")
        .attr("class", "tl-day-label")
        .attr("x", (d) => rescaled(d) + 2)
        .attr("y", 44)
        .style("fill", "var(--text-muted)")
        .style("font-size", "8px")
        .style("font-family", "ui-sans-serif, system-ui, sans-serif")
  ...
  if (visibleDays <= 90) {
    ...
    tickInterval = d3.timeMonth.every(
      Math.max(1, Math.round(visibleMonths / 8)),
    );
...
function draw() {
  ...
  const visibleSimNodes = simNodeData.filter((n) =>
  ...
  const maxConn =
    d3.max(
      visibleSimNodes.filter((n) => n.type === "note"),
      (d) => d.connectionCount,
    ) || 1;
  ...
  simulation = d3
    .forceSimulation(visibleSimNodes)
  ...
  const noteLabelEls = labelG
    .selectAll(".note-label")
    .data(visibleSimNodes.filter((n) => n.type === "note"))
    .join("text")
...
function autoFit(width, height, nodes) {
  ...
  const trimmed = fit.filter(
    (n) => Math.abs(n.x - mx) <= 2 * sx && Math.abs(n.y - my) <= 2 * sy,
  );
  ...
  const scale = Math.min(
    (width - pad * 2) / (xe[1] - xe[0] || 1),
    (height - pad * 2) / (ye[1] - ye[0] || 1),
    1.2,
  );
  svg
    .transition()
    ...
    .call(
      ...
      d3.zoomIdentity
        ...
        .scale(scale)
        .translate(-(xe[0] + xe[1]) / 2, -(ye[0] + ye[1]) / 2),
    );
}
...
function onHover(event, d) {
  ...
  tooltipX.value =
    rightX + cardW < window.scrollX + window.innerWidth
      ? rightX
...
function toggleFocus(d) {
  if (pathMode.value) {
    ...
    if (path) {
      ...
      pathResult.value = {
        nodeIds: new Set([pathFrom.value.id, d.id]),
        edgeKeys: new Set(),
        steps: -1,
      };
    }
...
function getNodeOpacity(d, matchedIds) {
  ...
  const isNeighbor = allLinks.some((l) => {
    ...
    const t = l.target?.id ?? l.target;
    return (
      (matchedIds.has(s) && t === d.id) || (matchedIds.has(t) && s === d.id)
    );
  });
  if (isNeighbor) return appearance.neighborOpacity;
  return selectionMode.value
    ? Math.max(appearance.neighborOpacity - 0.1, 0.1)
    : 0;
}
...
function updateOpacity() {
  ...
  svg
    .selectAll(".note-nodes circle")
    .attr("opacity", (d) => getNodeOpacity(d, m));
  ...
  svg
    .selectAll(".wiki-links line, .tag-links line")
    .attr("opacity", (d) => getLinkOpacity(d, m));
  // Highlight path source node with an indigo ring
  svg
    .selectAll(".note-nodes circle")
    .attr("stroke", (d) =>
      d.id === pathFrom.value?.id
        ? "#818cf8"
        : lightenHex(appearance.nodeColor, 40),
    )
...
watch(
  () => appearance.nodeColor,
  (v) => {
    svg
      ?.selectAll(".note-nodes circle")
      .attr("fill", v)
      .attr("stroke", lightenHex(v, 40));
  },
);
watch(
...
watch(
  () => appearance.edgeThickness,
  (v) => {
    svg?.selectAll(".wiki-links line").attr("stroke-width", v);
    svg
      ?.selectAll(".tag-links line")
      .attr("stroke-width", Math.max(v * 0.5, 0.8));
  },
);
watch(
  ...
  (v) => {
    svg
      ?.selectAll(".wiki-links line")
      .attr("marker-end", v ? "url(#arrow)" : null);
  },
);
watch(
  () => appearance.labelMode,
  (mode) => {
    svg
      ?.selectAll(".note-label, .tag-label")
      .attr("opacity", mode === "on" ? 1 : 0);
  },
);
...
watch(
  [
    () => appearance.repulsion,
    () => appearance.linkDistance,
    () => appearance.gravity,
  ],
  () => {
...
watch(
  () => appearance.neighborOpacity,
  () => updateOpacity(),
);

// Node size — needs redraw (scales change)
watch(
  () => appearance.minNodeSize,
  () => {
  ...
  searchDebounce = setTimeout(async () => {
    ...
    try {
      ...
      searchResults.value = res.results
        .map((r) => {
          const node = allNodes.find((n) => n.id === Number(r.id));
...
onUnmounted(() => {
  if (simulation) simulation.stop();
  if (timelineAxisGroup) timelineAxisGroup.remove();
});
</script>
...
<template>
  <div class="flex flex-col h-full bg-c-bg text-c-text">
    <!-- ── Controls bar ────────────────────────────────────────────────────── -->
    <div
      class="flex flex-wrap items-center gap-3 px-4 py-2.5 border-b border-c-border"
    >
      ...
      <button
        ...
        :title="
          selectionMode && customSelection.size > 0
            ? 'Focus on selected nodes'
            : 'Select nodes to view together'
        "
        @click="
          selectionMode && customSelection.size > 0
            ? applySelection()
      ...
      <div class="relative">
        <div class="flex items-center gap-1">
          ...
          <span
            v-if="searchLoading"
            class="text-xs text-c-text-muted animate-pulse"
            >…</span
          >
        </div>
        <div
        ...
        >
          ...
          >
            <div class="text-sm font-medium text-c-text truncate">
              {{ r.title }}
            </div>
            <div class="text-xs text-c-text-muted mt-0.5 truncate">
              {{ r.tags?.join(", ") }}
            </div>
          </button>
        </div>
      ...
      <!-- Orphans -->
      <label
        class="flex items-center gap-1.5 text-xs text-c-text-muted cursor-pointer select-none"
      >
      ...
      <div
        v-if="focusedNode"
        class="flex items-center gap-2 text-xs bg-blue-900/70 text-blue-200 px-2.5 py-1 rounded-full"
      >
        {{ focusedNode.title ?? focusedNode.name }}
        <button
          @click="focusedNode = null"
          class="text-blue-400 hover:text-white leading-none"
        >
          ✕
        </button>
      </div>
      <div
        v-if="focusedCluster"
        class="flex items-center gap-2 text-xs bg-emerald-900/70 text-emerald-200 px-2.5 py-1 rounded-full"
      >
        Cluster · {{ focusedCluster.size }} notes
        <button
          @click="focusedCluster = null"
          class="text-emerald-400 hover:text-white leading-none"
        >
          ✕
        </button>
      </div>
      <div
        v-if="selectionMode && customSelection.size > 0"
        class="flex items-center gap-2 text-xs bg-amber-900/70 text-amber-200 px-2.5 py-1 rounded-full"
      >
        {{ customSelection.size }} selected
        <button
          @click="selectionMode = false"
          class="text-amber-400 hover:text-white leading-none"
        >
          ✕
        </button>
      </div>
      <div
        v-if="pathMode && !pathFrom"
        class="text-xs text-c-text-muted italic"
      >
        Click a note to start path
      </div>
      <div
        v-if="pathFrom && !pathResult"
        class="flex items-center gap-2 text-xs bg-indigo-900/70 text-indigo-200 px-2.5 py-1 rounded-full"
      >
        From:
        {{
          pathFrom.title.length > 16
            ? pathFrom.title.slice(0, 16) + "…"
            : pathFrom.title
        }}
        · click destination
        <button
          @click="pathFrom = null"
          class="text-indigo-400 hover:text-white leading-none"
        >
          ✕
        </button>
      </div>
      ...
      >
        {{
          pathResult.steps >= 0
            ? `${pathResult.steps} step${pathResult.steps !== 1 ? "s" : ""}`
            : "No path found"
        }}
        <button
          @click="
          ...
          :class="pathResult.steps >= 0 ? 'text-indigo-400' : 'text-red-400'"
        >
          ✕
        </button>
      </div>
      ...
      <button
        @click="resetZoom"
        class="ml-auto text-xs text-c-text-muted hover:text-c-text px-2.5 py-1.5 rounded-lg border border-c-border hover:border-c-border-hover transition"
      >
        Reset zoom
      </button>
    </div>
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      >
        <component
          :is="statsOpen ? ChevronDown : ChevronUp"
          class="stats-handle-chevron"
        />
        ...
        <div class="stats-panel-body">
          <div class="stat-section">
            ...
            <div class="stat-row">
              <span
                class="stat-label"
                title="Note nodes currently visible in the graph"
                >Notes</span
              >
            ...
            <div class="stat-row" v-if="showTagNodes">
              <span
                class="stat-label"
                title="Tag nodes currently visible in the graph"
                >Tags</span
              >
            ...
            <div class="stat-row" v-if="showWikiLinks">
              <span
                class="stat-label"
                title="Direct note-to-note links visible in the graph"
                >Note links</span
              >
            ...
            <div class="stat-row" v-if="showTagNodes">
              <span
                class="stat-label"
                title="Note-to-tag connections visible in the graph"
                >Tag links</span
              >
            ...
            <div
              ...
              @click="lastUpdatedNote && focusOnNote(lastUpdatedNote)"
            >
              <span
                class="stat-label"
                title="Most recently edited note in the current view — click to focus it"
                >Last updated</span
              >
              <span class="stat-value">{{
                lastUpdatedNote
                  ? lastUpdatedNote.title.length > 18
                    ? lastUpdatedNote.title.slice(0, 18) + "…"
                    : lastUpdatedNote.title
                  : "—"
              }}</span>
            </div>
            <div class="stat-row">
              <span
                class="stat-label"
                title="Disconnected groups of notes — use arrows to cycle through each group"
                >Groups</span
              >
              <div class="flex items-center gap-1">
                <button
                  class="cluster-nav-btn"
                  :disabled="allClusters.length < 2"
                  @click="cycleCluster(-1)"
                >
                  ‹
                </button>
                <span class="stat-value">
                  {{ allClusters.length
                    ...
                    v-if="selectedClusterIndex !== null"
                    class="text-c-text-muted"
                  >
                    · {{ selectedClusterIndex + 1 }}</span
                  >
                </span>
                <button
                  class="cluster-nav-btn"
          ...
          <div class="stat-section">
            ...
            <div
              class="stat-row"
              :class="{ clickable: mostConnectedNote }"
              @click="mostConnectedNote && focusOnNote(mostConnectedNote)"
            >
              <span
                class="stat-label"
                title="Note with the most wiki-links — click to focus it"
                >Most connected</span
              >
              <span class="stat-value">{{
                mostConnectedNote
                  ? `${mostConnectedNote.title.slice(0, 18)}${mostConnectedNote.title.length > 18 ? "…" : ""} (${mostConnectedNote.degree})`
                  : "—"
              }}</span>
            </div>
            <div class="stat-row">
              <span
                class="stat-label"
                title="Notes with no links to or from any other note"
                >Isolated notes</span
              >
            ...
            <div
              class="stat-row"
              :class="{ clickable: sinkNotes.length > 0 }"
              @click="
                sinkNotes.length &&
                focusOnCluster(new Set(sinkNotes.map((n) => n.id)))
              "
            >
              <span
                class="stat-label"
                title="Notes that others link to, but that don't link out to anything — click to focus them"
                >Dead ends</span
              >
              <span class="stat-value">{{ sinkNotes.length || "—" }}</span>
            </div>
            <div class="stat-row">
              <span class="stat-label" title="Average number of links per note"
                >Avg connections</span
              >
              <span class="stat-value">{{ avgConnections }}</span>
            </div>
            <div
              class="stat-row clickable"
              @click="focusOnCluster(largestCluster.nodeIds)"
            >
              <span
                class="stat-label"
                title="Largest group of interconnected notes — click to focus it"
                >Biggest cluster</span
              >
          ...
          <div class="stat-section">
            ...
            <div class="stat-row">
              <span
                class="stat-label"
                title="Tag applied to the most notes across your entire graph"
                >Most used tag</span
              >
              <span v-if="mostUsedTag" class="tag-chip"
                >#{{ mostUsedTag.name }} ({{ mostUsedTag.count }})</span
              >
            ...
            <div class="stat-block">
              <div class="stat-row">
                <span
                  class="stat-label"
                  title="Tags that appear on only one note — niche or specific topics"
                  >Unique tags</span
                >
                <span class="stat-value">{{ nicheTags.length || "—" }}</span>
              </div>
              <div
                v-if="nicheTags.length"
                class="flex flex-wrap gap-1 mt-1 max-h-20 overflow-y-auto"
              >
                <span v-for="t in nicheTags" :key="t" class="tag-chip"
                  >#{{ t }}</span
                >
              </div>
            </div>
            <div
              class="stat-row"
              :class="{ clickable: mostVersatileNote }"
              @click="mostVersatileNote && focusOnNote(mostVersatileNote)"
            >
              <span
                class="stat-label"
                title="Note with the most tags — spans the most topics — click to focus it"
                >Most tagged note</span
              >
              <span class="stat-value">{{
                mostVersatileNote
                  ? `${mostVersatileNote.title.slice(0, 16)}${mostVersatileNote.title.length > 16 ? "…" : ""} (${mostVersatileNote.tags.length} tags)`
                  : "—"
              }}</span>
            </div>
            <div class="stat-row">
              <span
                class="stat-label"
                title="Tags that didn't exist before the last 30 days — recently introduced topics"
                >New tags (30d)</span
              >
      ...
      <div ref="containerRef" class="relative flex-1 overflow-hidden">
        <div
          v-if="loading"
          class="absolute inset-0 flex items-center justify-center text-c-text-muted text-sm"
        >
          Loading graph…
        </div>
        <div
      ...
      <Transition name="card-fade">
        ...
        >
          <template v-if="hoveredNode.type === 'note'">
            <p class="font-semibold text-c-text mb-1 truncate">
              {{ hoveredNode.title }}
            </p>
            <div class="flex flex-wrap gap-1 mb-2">
              <span v-for="tag in hoveredNode.tags" :key="tag" class="tag-chip"
                >#{{ tag }}</span
              >
            </div>
            <p class="text-c-text-muted text-xs">
              {{ hoveredNode.connectionCount }} connection{{
                hoveredNode.connectionCount !== 1 ? "s" : ""
              }}
            </p>
            <p
              v-if="timelineMode && hoveredNode.created_at"
              class="text-c-text-muted text-xs"
            >
              Created
              {{
                new Date(hoveredNode.created_at).toLocaleDateString("en-US", {
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                })
            ...
            <p
              v-else-if="hoveredNode.updated_at"
              class="text-c-text-muted text-xs mb-2"
            >
              Updated
              {{ new Date(hoveredNode.updated_at).toLocaleDateString() }}
            </p>
            <button
              @click="navigateToNote(hoveredNode.id)"
          ...
          <template v-else>
            ...
            <p class="text-c-text-muted text-xs">
              {{ hoveredNode.noteCount }} note{{
                hoveredNode.noteCount !== 1 ? "s" : ""
              }}
            </p>
          </template>
        </div>
    ...
    <div class="drawer" :class="{ open: drawerOpen }">
      ...
      <div class="drawer-handle" @click="drawerOpen = !drawerOpen">
        ...
        <component
          :is="drawerOpen ? ChevronDown : ChevronUp"
          class="w-4 h-4 text-c-text-muted ml-auto"
        />
      </div>
        ...
        <div class="appearance-grid">
          <div class="app-section">
            ...
            <label class="app-row">
              ...
              <input
                type="range"
                v-model.number="appearance.repulsion"
                min="50"
                max="2000"
                step="50"
                class="app-slider"
            ...
            <label class="app-row">
              ...
              <input
                type="range"
                v-model.number="appearance.linkDistance"
                min="50"
                max="500"
                step="10"
                class="app-slider"
            ...
            <label class="app-row">
              ...
              <input
                type="range"
                v-model.number="appearance.gravity"
                min="0"
                max="0.5"
                step="0.01"
                class="app-slider"
          ...
          <div class="app-section">
            ...
            <label class="app-row">
              ...
              <input
                type="range"
                v-model.number="appearance.minNodeSize"
                min="4"
                max="20"
                step="1"
                class="app-slider"
            ...
            <label class="app-row">
              ...
              <input
                type="range"
                v-model.number="appearance.neighborOpacity"
                min="0"
                max="1"
                step="0.05"
                class="app-slider"
              />
              <span class="app-val">{{
                appearance.neighborOpacity.toFixed(2)
              }}</span>
            </label>
            <label class="app-row">
          ...
          <div class="app-section">
            ...
            <label class="app-row">
              ...
              <input
                type="range"
                v-model.number="appearance.edgeThickness"
                min="0.5"
                max="8"
                step="0.5"
                class="app-slider"
            ...
            <label class="app-row">
              ...
              <input
                type="range"
                v-model.number="appearance.edgeOpacity"
                min="0.1"
                max="1"
                step="0.05"
                class="app-slider"
              />
              <span class="app-val">{{
                appearance.edgeOpacity.toFixed(2)
              }}</span>
            </label>
          ...
          <div class="app-section">
            ...
            <div class="flex gap-2 mt-1">
              <button
                v-for="mode in ['on', 'off']"
                :key="mode"
                class="label-mode-btn"
                :class="{ active: appearance.labelMode === mode }"
