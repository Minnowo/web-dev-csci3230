...
function drawMonth() {
  ...
  g.selectAll('.day-num').data(days).join('text')
    ...
    .text(d => d.getDate())
    .style('font-size', '10px').style('fill', '#94a3b8').style('pointer-events', 'none')
}
  ...
  <div class="flex flex-col h-full bg-gray-950 text-gray-100 p-4">
    ...
    <div class="flex flex-wrap items-center gap-4 mb-6">
      ...
      <div class="flex rounded-lg overflow-hidden border border-gray-700">
        <button
          @click="viewMode = 'year'"
          :class="viewMode === 'year' ? 'bg-gray-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'"
          class="px-3 py-1.5 text-sm transition"
        >Year</button>
        <button
          @click="viewMode = 'month'"
          :class="viewMode === 'month' ? 'bg-gray-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'"
          class="px-3 py-1.5 text-sm transition"
        >Month</button>
      </div>
